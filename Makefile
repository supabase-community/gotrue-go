...
up:
	...
	@docker compose -f integration_test/setup/docker-compose.yaml up -d 1>/dev/null 2>/dev/null && echo "${GREEN}Started${NC}"

down:
	@echo "${BLUE}Removing containers${NC}"
	@docker compose -f integration_test/setup/docker-compose.yaml down 2>/dev/null && echo "${GREEN}Removed${NC}"
